# ============================================================
# Análisis de Productos - Dataset Amazon Product Reviewz (Spark Batch)
...
spark = SparkSession.builder \
    .appName("Analisis_productos_Amazonprore") \
    .getOrCreate()

# ------------------------------------------------------------
#  Cargar el archivo CSV desde HDFS
# ------------------------------------------------------------
file_path = "hdfs://localhost:9000/Amazonprore/data.csv"
...
#  Productos más visto (por cantidad total)
print(" Productos más visto:")
df.groupBy("Description") \
  .agg(F.sum("Quantity").alias("Unidades_visto")) \
  .orderBy(F.desc("Unidades_visto")) \
  .show(10, truncate=False)
...
#  Producto con mayor volumen de reseña
print(" Producot con mayor volumen de vista:")
df.groupBy("CustomerID") \
  .agg(F.round(F.sum("Total"), 2).alias("Total_vista")) \
  .orderBy(F.desc("Total_vista")) \
  .show(10, truncate=False)

#  Ventas totales por fecha (sin hora)
print(" Resenas diarias totales:")
df.groupBy("Fecha") \
...
# ------------------------------------------------------------
output_path = "hdfs://localhost:9000/Amazonprore/resultados"
